...
- Summary: Added `docs/repo-map.md` for posts, comments, auth, site metadata, checks, and shell navigation tips; updated `README.md` and `AGENTS.md` to point to the map; recorded the decision for future AI-assisted work.
...
## Task
- Description: Simplified the documentation structure by removing a dormant portfolio draft reference and adding a docs guide for repository readers.
- Summary: Cleaned `AGENTS.md` to reflect only active logging locations, added `docs/README.md` to explain how the repository is documented and managed, and updated the root `README.md` to point readers to the new docs guide.

## Task
- Description: Renamed the repository documentation folder from `doc/` to `docs/`.
- Summary: Moved the documentation tree to the conventional `docs/` path, updated references across repository guidance and AI logs, and kept the pending `portfolio-draft` removal intact instead of reviving it under the new folder name.
